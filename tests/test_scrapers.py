...
async def test_playwright_scraper_live(pw_scraper, url):
    ...
    assert isinstance(result, dict), f"Result for {url} is not a dict"
    ...
    assert result.get("author"), f"Missing author for {url}"

    # Check for carousel items if it's a known carousel
    if "/p/" in url and (url == TEST_URLS[0] or url == TEST_URLS[3]):
        assert result["primary_media"]["media_type"] in ("GraphSidecar", "XDTGraphSidecar"), f"Expected sidecar for {url}"
